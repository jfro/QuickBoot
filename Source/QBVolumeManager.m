...
- (NSString *)helperPath
{
	...
	if([paths count] > 0)
		return [[[paths objectAtIndex:0] stringByAppendingPathComponent:@"QuickBoot"] stringByAppendingPathComponent:@"QuickBootHelper"];
	return nil;
}
...
//			NSLog(@"Removing helper...");
	...
	if([self passwordlessBootingEnabled])
	{
		if(![[NSFileManager defaultManager] fileExistsAtPath:[self helperPath]])
		{
			...
			[[NSFileManager defaultManager] copyItemAtPath:[[NSBundle mainBundle] pathForAuxiliaryExecutable:@"QuickBootHelper"]
